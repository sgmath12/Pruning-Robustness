...
idx_list = []
file_name = 'pgd_adversarial_training_vgg'
adv_accuracy = 0
for i,(x,y) in enumerate(test_loader):
    ...
    for k in range(31,32,1):
        ...
        if isinstance(vgg[1].features[k],nn.Conv2d):
            ...
            z_unknown = vgg(x_unknown)
            activations_unknown = result.features
            y_p = z_unknown.argmax()

            x_unknown_adv = attack(x_unknown,torch.unsqueeze(y_p,0))
            z_unknown_adv = vgg(x_unknown_adv)
            activations_unknown_adv = result.features
            

            diff_adv = torch.square(activations_unknown.squeeze() - activations_unknown_adv.squeeze())
            ...
            z_1 = vgg(x_adv)
            z_2 = vgg(x_adv_modified)
            pdb.set_trace()
            # print (y.item(),z_1.argmax().item(),z_2.argmax().item())
            # if z_1.argmax() != z_2.argmax():
            #     pdb.set_trace()
        

            print (k, y,z_unknown.argmax(dim = 1),z_after.argmax(dim = 1),)
